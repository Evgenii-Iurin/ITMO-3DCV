...
def run_single_test(data_dir, output_dir):
    from estimate_trajectory_archive import estimate_trajectory
    ...
    if len(results) == 0:
        ...
        for result in results:
            ...
            if inliers < testing.INLIERS_THRESHOLD:
                all_ok = False
                res = {"description": f"Too few inliers ({inliers})", "mark": 0}
                break
        ...
        if all_ok:
            ...
            res = {
                "description": f"tr {worst_translation:.2f} rot {worst_rotation:.2f}",
                "mark": f"{mark:.2f}",
            }
    if os.environ.get("CHECKER"):
        print(dumps(res))
