...
class AbsoluteTranslationalError:
    ...
    def estimate(test_trajectory, gt_trajectory):
        ...
        result.gt_from_test_transform, errors = Trajectory.align(
            test_trajectory, gt_trajectory
        )
    ...
    def estimate_from_cloud(self, cloud, search_tree):
        errors = [
            search_tree.get_distance_to_nearest(
                Trajectory.transform_point(self.gt_from_test_transform, point)
            )
            for point in cloud
        ]
        result = AbsoluteTranslationalError()
