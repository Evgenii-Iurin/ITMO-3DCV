...
class PointCloud:
    ...
    def generate_dataset_cloud(
        ...
        rgb_list = Dataset.read_dict_of_lists(
            Dataset.get_rgb_list_file(public_data_path)
        )
        depth_list = Dataset.read_dict_of_lists(
            Dataset.get_depth_list_file(private_data_path)
        )
        ...
        for frame_id, rgb_file in rgb_list.items():
            ...
            all_colors += colors
            print(
                "Generated cloud for frame {}/{} (points {})".format(
                    frame_id + 1, len(rgb_list), len(all_points)
                )
            )
    ...
    def write_ply(filename, points, colors):
        ...
        with open(filename, "w") as file:
            ...
            for point, color in zip(points, colors):
                file.write(
                    "{} {}\n".format(
                        " ".join(map(str, point)), " ".join(map(str, color))
                    )
                )
