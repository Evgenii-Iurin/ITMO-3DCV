...
class RelativePoseError:
    ...
    def estimate(test_trajectory, gt_trajectory):
        ...
        for id in range(0, len(gt_trajectory) - RelativePoseError.CHECK_WINDOW_WIDTH):
            ...
            test_relative = Trajectory.compute_relative_transform(
                test_trajectory[key], test_trajectory[ref_key]
            )
            gt_relative = Trajectory.compute_relative_transform(
                gt_trajectory[key], gt_trajectory[ref_key]
            )

            difference = Trajectory.compute_relative_transform(
                gt_relative, test_relative
            )
        ...
        result.translation_rmse = numpy.sqrt(
            numpy.dot(translation_errors, translation_errors) / len(translation_errors)
        )
        ...
        result.rotation_rmse = numpy.degrees(
            numpy.sqrt(
                numpy.dot(rotation_errors, rotation_errors) / len(rotation_errors)
            )
        )
