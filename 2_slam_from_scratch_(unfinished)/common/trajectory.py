...
class Trajectory:
    ...
    def read(filename, key_type=int):
        ...
        for key, pose_str in trajectory_dict.items():
            ...
            for value in pose:
                assert not numpy.isnan(value), 'Line "{}" of file "{}" has NaNs'.format(
                    pose_str, filename
                )
    ...
    def write(filename, trajectory):
        with open(filename, "w") as file:
            ...
            for frame_id, pose in trajectory.items():
                file.write(
                    "{} {}\n".format(
                        frame_id, " ".join(map(str, Trajectory.from_matrix4(pose)))
                    )
                )
    ...
    def get_translations(trajectory):
        return numpy.matrix(
            [matrix[0:3, 3] for matrix in trajectory.values()]
        ).transpose()
    ...
    def compute_median_distance_between_adjacent_frames(trajectory):
        ...
        for id in range(0, len(trajectory) - 1):
            difference = Trajectory.compute_relative_transform(
                trajectory[keys[id]], trajectory[keys[id + 1]]
            )
    ...
    @staticmethod
    def estimate_unknown_camera_poses(rmat, tvec):
