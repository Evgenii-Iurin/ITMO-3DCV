...
def compute_inliers_score(test_trajectory, gt_trajectory, all_tests):
    ...
    for key, test in test_trajectory.items():
        ...
        if (
            Trajectory.compute_angle(diff) < ANGLE_THRESHOLD
            and Trajectory.compute_distance(diff) < DISTANCE_THRESHOLD
        ):
            inliers.append(key)
...
def test_result(
    test_data_path, private_data_path, result_filename, test_inliers_only=False
):
    ...
    test_trajectory = {
        key: value for key, value in test_trajectory.items() if key not in ignored_ids
    }
    gt_trajectory = {
        key: value for key, value in gt_trajectory.items() if key in test_trajectory
    }

    inliers_score, inliers = compute_inliers_score(
        test_trajectory, gt_trajectory, unknown_poses_num
    )

    if test_inliers_only:
        test_trajectory = {
            key: value for key, value in test_trajectory.items() if key in inliers
        }
        gt_trajectory = {
            key: value for key, value in gt_trajectory.items() if key in test_trajectory
        }
